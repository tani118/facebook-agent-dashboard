  ...
  async fetchPostComments(postId, limit = 25, order = 'chronological', includeReplies = true) {
    try {
      ...
      const params = {
        ...
        limit: limit,
        order: order,
        fields: 'id,message,created_time,from{id,name,picture.type(large)},attachment,like_count,comment_count,parent,user_likes,can_reply_privately,permalink_url'
      };

      // Include comment replies if requested with proper syntax
      if (includeReplies) {
        params.fields += ',comments.limit(50){id,message,created_time,from{id,name,picture.type(large)},attachment,like_count,parent,permalink_url}';
      }
  ...
  async fetchComment(commentId) {
    try {
      ...
      const response = await axios.get(url, {
        params: {
          access_token: this.pageAccessToken,
          fields: 'id,message,created_time,from{id,name,picture},attachment,like_count,comment_count,parent,user_likes,can_reply_privately,permalink_url'
        }
      });
